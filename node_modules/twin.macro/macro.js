...
(selector, {
  selectorMatchReg,
  sassyPseudo
}) => selector.replace(selectorMatchReg, (match, __, offset) => selector === match || offset === 0 && !sassyPseudo ? '' : '&'), // Remove unneeded escaping from the selector
...
const HANDLED_ATRULE_TYPES = new Set(['document', 'media', 'page', 'screen', 'supports']);
...
function extractFromRule(rule, params) {
  ...
  const selector = unescape(selectorForUnescape).replace(ESC_COMMA, ',') // Remove comma escaping
  .replace(LINEFEED$1, ' ');
...
const ruleTypes = {
  ...
  rule(rule, params) {
    ...
    params.debug('sassified key', (_selector = selector) != null ? _selector : styles);
...
function spreadVariantGroups(classes, context) {
  ...
  const group = classList.map(className => {
    ...
    if (BRACKETED_MAYBE_IMPORTANT.test(className)) {
      ...
      return expandVariantGroups([...pieces, className].join((_context$tailwindConf2 = context.tailwindConfig.separator) != null ? _context$tailwindConf2 : ':'), ctx);
    }

    return [...pieces, [beforeImportant, className, afterImportant].join('')].filter(Boolean).join((_context$tailwindConf3 = context.tailwindConfig.separator) != null ? _context$tailwindConf3 : ':');
  }).filter(Boolean);
  return group;
}

function expandVariantGroups(classes, context) {
  const classList = [...splitAtTopLevelOnly(classes.replace(NEW_LINE, ' ').trim(), ' ')];
...
function convertShortCssToArbitraryProperty(className, {
  ...
  assert(!isShortCssDisabled, ({
    color
  }) => [`${String(color(`✕ ${String(color(className, 'errorLight'))} uses twin’s deprecated short-css syntax`))}`, `Update to ${String(color(arbitraryProperty, 'success'))}`, `To ignore this notice, add this to your twin config:\n{ "disableShortCss": false }`, `Read more at https://twinredirect.page.link/short-css`].join('\n\n'));
...
function convertClassName(className, {
  ...
  if (className.endsWith('!')) {
    var _tailwindConfig$separ4, _tailwindConfig$separ5;

    debug('trailing bang found', className);
    const splitArray = [...splitAtTopLevelOnly(className.slice(0, -1), (_tailwindConfig$separ4 = tailwindConfig.separator) != null ? _tailwindConfig$separ4 : ':')]; // Place a ! before the class

    splitArray.splice(-1, 1, `!${splitArray[splitArray.length - 1]}`);
    className = splitArray.join((_tailwindConfig$separ5 = tailwindConfig.separator) != null ? _tailwindConfig$separ5 : ':');
  ...
  className = checkForMissingParentSelector(className, {
    tailwindConfig
  });
...
function checkForMissingParentSelector(fullClassName, {
  tailwindConfig
}) {
  var _tailwindConfig$separ6, _tailwindConfig$separ7;

  const splitArray = [...splitAtTopLevelOnly(fullClassName, (_tailwindConfig$separ6 = tailwindConfig.separator) != null ? _tailwindConfig$separ6 : ':')];
  const variants = splitArray.slice(0, -1);
  const className = splitArray.slice(-1)[0];
  const arbitraryVariantsCount = variants.filter(v => v.startsWith('[') && v.endsWith(']')).length;
  if (arbitraryVariantsCount === 0) return fullClassName;
  const variantsNew = variants.map((v, offset) => {
    if (!(v.startsWith('[') && v.endsWith(']'))) return v; // Missing parent selector found

    const unwrapped = v.slice(1, -1);
    const added = addParentSelector(unwrapped, offset, arbitraryVariantsCount);
    return `[${added}]`;
  });
  return [...variantsNew, className].join((_tailwindConfig$separ7 = tailwindConfig.separator) != null ? _tailwindConfig$separ7 : ':');
}

function addParentSelector(value, offset, arbitraryVariantsCount) {
  // Tailwindcss requires pre-encoded commas - unencoded are removed and we end up with an invalid selector
  const selector = escapeCommas(value); // Preserve selectors with parent selector or media queries

  if (selector.includes('&') || selector.startsWith('@')) return selector; // pseudo

  if (selector.startsWith(':')) return `&${selector}`; // Selectors with multiple arbitrary variants are too hard to determine so follow a basic rule instead

  if (arbitraryVariantsCount > 1) return `&_${selector}`; // If the arbitrary variant is the first selector, add a parent selector
...
function validateClasses(classes, {
  ...
  for (const className of splitAtTopLevelOnly(classes, ' ')) {
    ...
    assert(!className.endsWith((_tailwindConfig$separ = tailwindConfig.separator) != null ? _tailwindConfig$separ : ':'), ({
      color
    }) => `${color(`✕ The variant ${String(color(className, 'errorLight'))} doesn’t look right`)}\n\nUpdate to ${String(color(`${className}block`, 'success'))} or ${String(color(`${className}(block mt-4)`, 'success'))}`);
  }

  return true;
}

const tasks = [classes => classes.replace(CLASS_DIVIDER_PIPE, ' '), classes => classes.replace(COMMENTS_MULTI_LINE, multilineReplaceWith), classes => classes.replace(COMMENTS_SINGLE_LINE, ''), (classes, tailwindConfig) => expandVariantGroups(classes, {
  tailwindConfig
